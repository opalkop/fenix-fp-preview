...
const FenixCore=(()=>{
  ...
  function normalizeProject(project={}){const createdAt=project.createdAt||now(),assets=normalizeAssets(project.assets),contentPlan=normalizeContentPlan(project.contentPlan);return{id:project.id||uid(),name:String(project.name||"Nowy projekt").trim()||"Nowy projekt",format:normalizeFormat(project.format),bleed:project.bleed||"no-bleed",ageGroup:project.ageGroup||"",topic:project.topic||"",createdAt,updatedAt:project.updatedAt||createdAt,pages:Array.isArray(project.pages)?project.pages:[],assets,contentPlan}}
  ...
  function updateProject(id,patch={}){const current=ensureSchema(),index=current.projects.findIndex(project=>project.id===id);if(index<0)return false;const existing=current.projects[index],pages=Array.isArray(patch.pages)?patch.pages.map(normalizePage):existing.pages,contentPlan=normalizeContentPlan(patch.contentPlan??existing.contentPlan);current.projects[index]=normalizeProject({...existing,...patch,id:existing.id,createdAt:existing.createdAt,updatedAt:now(),pages,contentPlan});commit({projects:true,cart:Array.isArray(patch.pages),assets:Boolean(patch.assets),contentPlan:Boolean(patch.contentPlan)});return clone(current.projects[index])}
