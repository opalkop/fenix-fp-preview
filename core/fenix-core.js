...
const FenixCore=(()=>{
  ...
  function exportPack(){const project=getActiveProject();download(`${safeName(project.name)}-${new Date().toISOString().slice(0,10)}.fenixpack`,JSON.stringify({type:"FENIX_PACK",version:4,schemaVersion:window.FenixPageSchema?.VERSION||3,project:{...project,pages:undefined},pages:project.pages,assets:project.assets},null,2))}
  function exportProject(){const project=getActiveProject();download(`${safeName(project.name)}.fenixproject`,JSON.stringify({type:"FENIX_PROJECT",version:3,schemaVersion:window.FenixPageSchema?.VERSION||3,project},null,2))}
