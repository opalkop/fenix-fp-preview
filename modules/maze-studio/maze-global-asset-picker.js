...
(()=>{
  ...
  function rebuildOne(id,preferred=""){
    ...
    for(const [pack,assets] of groups){
      html+=`<optgroup label="${esc(pack)}">`;
      for(const asset of assets){const localId=linkedLocalId(asset.id),value=localId||`${PREFIX}${asset.id}`;html+=`<option value="${esc(value)}">${esc(asset.name||asset.filename||asset.id)}</option>`}
      html+='</optgroup>';
    }
    select.innerHTML=html;
    if([...select.options].some(option=>option.value===current))select.value=current;
    else if(current){
      const local=FenixCore.getAsset?.(current),libRef=refOf(local);if(libRef){const fallback=linkedLocalId(libRef)||`${PREFIX}${libRef}`;if([...select.options].some(option=>option.value===fallback))select.value=fallback}
    }
  }
