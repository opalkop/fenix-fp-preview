...
window.FenixEndingRenderers=(()=>{
  const DEFINITIONS=Object.freeze({
    "congratulations-studio":{
      ...
      defaults:{title:"CONGRATULATIONS!",subtitle:"YOU DID IT!",body:"You completed every challenge and reached the end of this adventure. Be proud of your focus, creativity, and determination!",footer:"Amazing work — keep exploring!",subtitleSize:52,bodySize:56,detailSize:42,footerSize:44,showActivityCount:true,countMode:"auto",activityCount:35,activityCountText:"You completed {count} activities!",showSkills:false,skillsText:"Focus · Observation · Problem-solving · Creativity",showQrTransition:true,transitionTitle:"YOUR ADVENTURE DOESN’T HAVE TO END HERE!",transitionText:"Turn the page to discover what comes next.",alignment:"center",style:"framed",titleSize:"standard",spacing:"standard",showAchievementMark:true},
      groups:[
    ...
    "certificate-studio":{
      ...
      groups:[
        ...
        {title:"3. Pola do uzupełnienia",hint:"Data i podpis rodzica lub nauczyciela pozostają polami do ręcznego uzupełnienia.",fields:[["showDate","Pokaż pole daty","checkbox","Dodaje linię do ręcznego wpisania daty."],["dateLabel","Podpis pola daty","text",60],["showSignature","Pokaż pole podpisu rodzica / nauczyciela","checkbox","Dodaje niezależną pustą linię do ręcznego podpisania."],["signatureLabel","Podpis pola podpisu","text",90],["footer","Końcowa stopka","text",180]]},
        {title:"4. Creator Mark",hint:"Dodatkowy znak autora lub marki — niezależny od podpisu rodzica i nauczyciela.",fields:[["showCreatorMark","Pokaż Creator Mark","checkbox","Umieszcza znak autora pod polami daty i podpisu."],["creatorMarkAssetRef","Asset Creator Mark","creator-asset","Opcjonalny znak PNG, JPG, WEBP lub SVG. Najlepiej użyć pliku z przezroczystym albo białym tłem."],["creatorMarkSize","Rozmiar Creator Mark","select",[["small","Mały"],["medium","Średni — zalecany"],["large","Duży"]]]]},
        {title:"5. Wygląd i wydruk",hint:"Czarno-biały certyfikat bez warstwy deco; Book Builder umieści go na stronie parzystej.",fields:[["style","Ramka certyfikatu","select",[["minimal","Bez ramki"],["classic","Klasyczna ramka"],["double-frame","Podwójna ramka"]]],["titleSize","Rozmiar nagłówka","select",[["small","Mniejszy"],["standard","Standardowy"],["large","Duży"]]],["spacing","Odstępy","select",[["compact","Kompaktowe"],["standard","Standardowe"],["airy","Przestronne"]]],["showAchievementMark","Pokaż symbol ukończenia","checkbox","Prosty czarno-biały znak ✓ — bez warstwy deco."],["showCutGuide","Pokaż linię wycięcia","checkbox","Delikatna przerywana linia przy krawędzi strony."],["bodySize","Wielkość treści certyfikatu","number",[40,90]],["labelSize","Wielkość podpisów pól","number",[28,64]],["footerSize","Wielkość stopki","number",[28,72]]]}
      ]
    },
    "qr-studio":{
  ...
  function renderCongratulations(settings,width,height){
    const {canvas,ctx,s}=base(width,height,settings.style),left=settings.alignment==="left",subtitleSize=Math.max(34,Math.min(90,Number(settings.subtitleSize)||52)),bodySize=Math.max(40,Math.min(90,Number(settings.bodySize)||56)),detailSize=Math.max(30,Math.min(72,Number(settings.detailSize)||42)),footerSize=Math.max(28,Math.min(72,Number(settings.footerSize)||44)),x=left?245*s:width/2,align=left?"left":"center",max=2060*s,gap={compact:.78,standard:1,airy:1.22}[settings.spacing]||1,titleSize={small:88,standard:108,large:128}[settings.titleSize]||108;
    ...
    if(settings.subtitle){y+=45*s*gap;ctx.font=`900 ${subtitleSize*s}px Arial, sans-serif`;ctx.fillText(settings.subtitle,x,y);y+=65*s}
    y+=55*s*gap;ctx.lineWidth=5*s;ctx.beginPath();ctx.moveTo(left?x:x-320*s,y);ctx.lineTo(left?x+640*s:x+320*s,y);ctx.stroke();
    y=textBlock(ctx,settings.body,x,y+125*s*gap,max,bodySize*s,(bodySize*1.5)*s,align,7);
    if(settings.showActivityCount){y+=75*s*gap;const message=String(settings.activityCountText||"You completed {count} activities!").replaceAll("{count}",String(Math.max(1,Number(settings.activityCount)||1)));ctx.font=`900 ${Math.min(72,detailSize+10)*s}px Arial, sans-serif`;wrap(ctx,message,max).slice(0,2).forEach((line,index)=>ctx.fillText(line,x,y+index*65*s));y+=Math.min(2,wrap(ctx,message,max).length)*65*s}
    if(settings.showSkills&&settings.skillsText){y+=65*s*gap;ctx.font=`800 ${Math.max(28,detailSize-8)*s}px Arial, sans-serif`;ctx.fillText("SKILLS PRACTICED",x,y);y=textBlock(ctx,settings.skillsText,x,y+60*s,max,detailSize*s,(detailSize*1.45)*s,align,3)}
    if(settings.showQrTransition){y=Math.max(y+95*s*gap,1980*s);ctx.lineWidth=3*s;ctx.beginPath();ctx.moveTo(left?x:x-250*s,y);ctx.lineTo(left?x+500*s:x+250*s,y);ctx.stroke();y+=90*s*gap;ctx.font=`900 ${detailSize*s}px Arial, sans-serif`;wrap(ctx,settings.transitionTitle,max).slice(0,2).forEach((line,index)=>ctx.fillText(line,x,y+index*55*s));y+=Math.min(2,wrap(ctx,settings.transitionTitle,max).length)*55*s+35*s;textBlock(ctx,settings.transitionText,x,y,max,detailSize*s,(detailSize*1.5)*s,align,4)}
    ctx.textAlign="center";ctx.font=`700 ${footerSize*s}px Arial, sans-serif`;ctx.fillText(settings.footer,width/2,height-270*s);return canvas;
  }
  function renderCertificate(settings,width,height,creatorMarkImage=null){
    const {canvas,ctx,s}=base(width,height,settings.style),cx=width/2,bodySize=Math.max(40,Math.min(90,Number(settings.bodySize)||54)),labelSize=Math.max(28,Math.min(64,Number(settings.labelSize)||38)),footerSize=Math.max(28,Math.min(72,Number(settings.footerSize)||40)),gap={compact:.86,standard:1,airy:1.14}[settings.spacing]||1,titleSize={small:86,standard:105,large:122}[settings.titleSize]||105;
    ...
    ctx.font=`400 ${Math.max(36,bodySize-8)*s}px Arial, sans-serif`;wrap(ctx,settings.preamble,1740*s).slice(0,2).forEach((line,index)=>ctx.fillText(line,cx,y+index*62*s));y+=150*s*gap;
    ctx.lineWidth=4*s;ctx.beginPath();ctx.moveTo(520*s,y);ctx.lineTo(2030*s,y);ctx.stroke();ctx.font=`italic ${labelSize*s}px Arial, sans-serif`;ctx.fillText(settings.nameLabel||"Name",cx,y+58*s);y+=220*s*gap;
    y=textBlock(ctx,settings.achievement,cx,y,1760*s,bodySize*s,(bodySize*1.48)*s,"center",5)+45*s*gap;
    if(settings.showBookTitle&&settings.bookTitle){ctx.font=`900 ${Math.max(42,bodySize-4)*s}px Arial, sans-serif`;const bookLines=wrap(ctx,settings.bookTitle,1740*s).slice(0,2);bookLines.forEach((line,index)=>ctx.fillText(line,cx,y+index*62*s));y+=bookLines.length*62*s+35*s*gap}
    if(settings.showActivityCount){const message=String(settings.activityCountText||"Completed {count} activities").replaceAll("{count}",String(Math.max(1,Number(settings.activityCount)||1)));ctx.font=`700 ${Math.max(34,labelSize+4)*s}px Arial, sans-serif`;wrap(ctx,message,1680*s).slice(0,2).forEach((line,index)=>ctx.fillText(line,cx,y+index*52*s))}
    const fieldY=2500*s;ctx.lineWidth=3*s;ctx.font=`400 ${labelSize*s}px Arial, sans-serif`;if(settings.showDate&&settings.showSignature){ctx.beginPath();ctx.moveTo(390*s,fieldY);ctx.lineTo(1080*s,fieldY);ctx.moveTo(1470*s,fieldY);ctx.lineTo(2160*s,fieldY);ctx.stroke();ctx.fillText(settings.dateLabel,735*s,fieldY+60*s);ctx.fillText(settings.signatureLabel,1815*s,fieldY+60*s)}else if(settings.showDate){ctx.beginPath();ctx.moveTo(765*s,fieldY);ctx.lineTo(1785*s,fieldY);ctx.stroke();ctx.fillText(settings.dateLabel,cx,fieldY+60*s)}else if(settings.showSignature){ctx.beginPath();ctx.moveTo(765*s,fieldY);ctx.lineTo(1785*s,fieldY);ctx.stroke();ctx.fillText(settings.signatureLabel,cx,fieldY+60*s)}
    if(settings.showCreatorMark&&creatorMarkImage){const sizes={small:[300,110],medium:[430,150],large:[560,190]},[maxWidth,maxHeight]=sizes[settings.creatorMarkSize]||sizes.medium,sourceWidth=creatorMarkImage.naturalWidth||creatorMarkImage.width||1,sourceHeight=creatorMarkImage.naturalHeight||creatorMarkImage.height||1,scale=Math.min(maxWidth*s/sourceWidth,maxHeight*s/sourceHeight),drawWidth=sourceWidth*scale,drawHeight=sourceHeight*scale;ctx.save();ctx.filter="grayscale(1) contrast(1.2)";ctx.drawImage(creatorMarkImage,cx-drawWidth/2,2760*s-drawHeight/2,drawWidth,drawHeight);ctx.restore()}
    ctx.font=`700 ${footerSize*s}px Arial, sans-serif`;wrap(ctx,settings.footer,1800*s).slice(0,2).forEach((line,index)=>ctx.fillText(line,cx,height-285*s+index*48*s));return canvas;
  }
