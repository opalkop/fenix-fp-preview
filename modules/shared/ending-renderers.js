...
window.FenixEndingRenderers=(()=>{
  const DEFINITIONS=Object.freeze({
    ...
    "certificate-studio":{
      ...
      groups:[
        ...
        {title:"3. Pola do uzupełnienia",hint:"Wybierz pola, które dziecko lub opiekun uzupełni po wydrukowaniu.",fields:[["showDate","Pokaż pole daty","checkbox","Dodaje linię do ręcznego wpisania daty."],["dateLabel","Podpis pola daty","text",60],["showSignature","Pokaż pole podpisu","checkbox","Dodaje linię na podpis rodzica, opiekuna lub nauczyciela."],["signatureLabel","Podpis pola podpisu","text",90],["signatureAssetRef","Asset podpisu","signature-asset","Opcjonalny podpis PNG, JPG, WEBP lub SVG. Najlepiej użyć pliku z przezroczystym albo białym tłem."],["footer","Końcowa stopka","text",180]]},
  ...
  function renderCertificate(settings,width,height,signatureAssetImage=null){
    ...
    const fieldY=2550*s,drawSignature=(center,maxWidth,maxHeight)=>{if(!signatureAssetImage)return;const sourceWidth=signatureAssetImage.naturalWidth||signatureAssetImage.width||1,sourceHeight=signatureAssetImage.naturalHeight||signatureAssetImage.height||1,scale=Math.min(maxWidth/sourceWidth,maxHeight/sourceHeight),drawWidth=sourceWidth*scale,drawHeight=sourceHeight*scale;ctx.save();ctx.filter="grayscale(1) contrast(1.2)";ctx.drawImage(signatureAssetImage,center-drawWidth/2,fieldY-drawHeight-18*s,drawWidth,drawHeight);ctx.restore()};ctx.lineWidth=3*s;ctx.font=`400 ${32*s}px Arial, sans-serif`;if(settings.showDate&&settings.showSignature){drawSignature(1815*s,560*s,180*s);ctx.beginPath();ctx.moveTo(390*s,fieldY);ctx.lineTo(1080*s,fieldY);ctx.moveTo(1470*s,fieldY);ctx.lineTo(2160*s,fieldY);ctx.stroke();ctx.fillText(settings.dateLabel,735*s,fieldY+60*s);ctx.fillText(settings.signatureLabel,1815*s,fieldY+60*s)}else if(settings.showDate){ctx.beginPath();ctx.moveTo(765*s,fieldY);ctx.lineTo(1785*s,fieldY);ctx.stroke();ctx.fillText(settings.dateLabel,cx,fieldY+60*s)}else if(settings.showSignature){drawSignature(cx,760*s,210*s);ctx.beginPath();ctx.moveTo(765*s,fieldY);ctx.lineTo(1785*s,fieldY);ctx.stroke();ctx.fillText(settings.signatureLabel,cx,fieldY+60*s)}
  ...
  function fromPage(page,module){const definition=DEFINITIONS[module],stored=object(page?.recipe?.settings),content=object(page?.recipe?.content);return{...definition.defaults,...stored,...(module==="qr-studio"&&content.assetRef?{qrAssetRef:content.assetRef}:{}),...(module==="certificate-studio"&&content.signatureAssetRef?{signatureAssetRef:content.signatureAssetRef}:{})}}
  function page(module,settings,original=null){
    const definition=DEFINITIONS[module],stamp=new Date().toISOString();
    const assetRef=module==="qr-studio"?String(settings.qrAssetRef||""):"",signatureAssetRef=module==="certificate-studio"?String(settings.signatureAssetRef||""):"",content={};if(assetRef)content.assetRef=assetRef;if(signatureAssetRef)content.signatureAssetRef=signatureAssetRef;
    return FenixPageSchema.normalize({id:original?.id,createdAt:original?.createdAt||stamp,updatedAt:stamp,module,title:settings.title||definition.label,recipe:{module,seed:null,title:settings.title||definition.label,settings:{...definition.defaults,...settings},content,meta:{renderer:"ending-v2"},renderState:{}},solution:{available:false,imageData:null},validation:{kdp:{status:module==="qr-studio"&&!assetRef?"warning":"ok",messages:module==="qr-studio"&&!assetRef?["Dodaj asset kodu QR przed eksportem finalnego PDF."]:[]}},production:{format:"8.5x11",bleed:"no-bleed",dpi:300,width:2550,height:3300},source:{app:module,version:"0.26.1",format:"native"}});
  }
  function render(pageValue,{width=2550,height=3300,qrAssetImage=null,signatureAssetImage=null}={}){
    ...
    if(module==="certificate-studio")return renderCertificate(settings,width,height,signatureAssetImage);
