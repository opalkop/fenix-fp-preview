...
(()=>{
  ...
  FenixCore.addPage=function(payload){
    if(FenixPageSchema.moduleOf(payload)==="matching-studio"){
  ...
  const page=FenixCore.getCart().find(item=>item.id===id&&FenixPageSchema.moduleOf(item)==="matching-studio");if(!page)return;
