...
(()=>{
  ...
  function coloringPages(){return FenixCore.getCart().map(FenixPageSchema.normalize).filter(page=>FenixPageSchema.moduleOf(page)==="coloring-studio")}
  ...
  function renderStudioPages(){
    state.assets=eligibleAssets();const pages=coloringPages(),list=$("#studioPagesList"),count=$("#studioPagesCount");count.textContent=`${pages.length} ${pages.length===1?"strona":"stron"}`;
    if(!pages.length){list.innerHTML='<div class="studio-pages-empty">Nie ma jeszcze zapisanych stron Coloring Studio.</div>';return}
    list.innerHTML=pages.map((page,index)=>{const asset=resolveSavedAsset(page),resolved=assetWithPayload(asset),assetName=asset?.name||identityFromPage(page).name||"Brak przypisanego assetu",src=resolved?.dataUrl||"",current=page.id===state.editingId;return `<article class="studio-page-item${current?" current":""}" data-page-id="${escapeHtml(page.id)}"><div class="studio-page-thumb">${src?`<img src="${src}" alt="${escapeHtml(assetName)}">`:"—"}</div><div class="studio-page-copy"><strong>Coloring #${String(index+1).padStart(2,"0")} · ${escapeHtml(page.title||"Color the Picture!")}</strong><span>${escapeHtml(assetName)}${current?" · EDYCJA":""}</span></div><div class="studio-page-actions"><button type="button" class="secondary" data-edit-page="${escapeHtml(page.id)}">Edytuj</button><button type="button" class="secondary" data-remove-page="${escapeHtml(page.id)}">Usuń</button></div></article>`}).join("");
    list.querySelectorAll("[data-edit-page]").forEach(button=>button.onclick=()=>openSavedPage(button.dataset.editPage));
    list.querySelectorAll("[data-remove-page]").forEach(button=>button.onclick=()=>removeSavedPage(button.dataset.removePage))
  }
  ...
  async function makePage(asset,original=null){
    ...
    const page=FenixPageSchema.normalize({id:original?.id,createdAt:original?.createdAt,module:"coloring-studio",title:o.title,recipe:{module:"coloring-studio",seed:null,title:o.title,settings:{...o,pageIndex:0,lockedAssetRef:resolved.id,assetLibraryRef:libRef||null},content:{assetRef:resolved.id,assetLibraryRef:libRef||null,assetName:resolved.name||null,assetFilename:resolved.filename||null},meta:{...(original?.recipe?.meta||{}),createdWith:"FENIX PC",moduleVersion:8,assetLibraryRef:libRef||null,assetName:resolved.name||null,assetFilename:resolved.filename||null}},preview:{imageData:null},solution:{available:false,imageData:null},validation:{kdp:resolved.validation||{status:"unknown",messages:[]}},source:{app:"fenix-desktop",version:"0.31.0",format:"native"}});
  ...
  function saveToProject(){
    ...
    if(state.editingId){FenixCore.updatePage(state.editingId,item.page);renderStudioPages();setStatus(`Zapisano stronę z assetem „${item.asset.name}”. Asset pozostaje przypisany.`);return}
  ...
  async function restoreEditedPage(){if(!state.editingId)return;await FenixCore.ready;projectInfo();state.assets=eligibleAssets();const restored=loadSavedPage();renderPicker();renderStudioPages();if(restored&&state.lockedAssetId)await build();else{state.pages=[];show();setStatus("Nie udało się jednoznacznie odnaleźć assetu przypisanego do tej zapisanej strony. Nie wybrano losowego zamiennika.",true)}}
  function scheduleRestore(){clearTimeout(restoreTimer);restoreTimer=setTimeout(()=>void restoreEditedPage(),20)}
  function startNewPage(){state.editingId=null;state.selectedAssetId=null;state.lockedAssetId=null;state.pages=[];history.replaceState(null,"",location.pathname);$("#cart").textContent="Dodaj stronę do projektu";projectInfo();renderPicker();renderStudioPages();show();setStatus("Nowa strona — wybierz jeden asset z listy.")}
  ...
  function openSavedPage(id){if(!id)return;state.editingId=id;state.selectedAssetId=null;state.lockedAssetId=null;state.pages=[];history.replaceState(null,"",`${location.pathname}?id=${encodeURIComponent(id)}`);void restoreEditedPage()}
  function removeSavedPage(id){if(!id)return;const page=coloringPages().find(item=>item.id===id);if(!page)return;if(!confirm(`Usunąć stronę „${page.title||"Coloring"}” z projektu?`))return;FenixCore.removePage(id);if(state.editingId===id)startNewPage();else renderStudioPages()}
  ...
  window.addEventListener("fenix-state-change",event=>{const detail=event.detail||{};if(detail.cart)renderStudioPages();if(detail.assets||detail.activeProject){projectInfo();renderStudioPages();if(state.editingId){scheduleRestore();return}renderPicker();if(state.lockedAssetId)void build();else show()}});
  window.addEventListener("fenix-storage-ready",()=>{renderStudioPages();if(state.editingId){scheduleRestore();return}renderPicker();if(state.lockedAssetId)void build();else show()});
  async function init(){projectInfo();setStatus("Wczytuję projekt i assety…");await FenixCore.ready;state.assets=eligibleAssets();renderStudioPages();if(state.editingId){await restoreEditedPage();return}renderPicker();show();setStatus("Wybierz jeden asset z listy. Po wyborze zostanie przypisany do strony.")}
  void init();
})();