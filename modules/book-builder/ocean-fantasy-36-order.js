...
(()=>{
  ...
  const VERSION="ocean-fantasy-36-v2";
  ...
  async function run(){
    try{
      ...
      if(summary&&result.applied)summary.textContent=`Ułożono i zapisano ${result.count} aktywności Ocean Fantasy zgodnie z planem przygody.`;
    }catch(error){
      console.error("FENIX Ocean Fantasy order bootstrap failed",error);
    }
  }

  window.FenixOceanFantasyOrderV2=Object.freeze({PROJECT_ID,VERSION,ORDER:[...ORDER],buildReorderedPages,apply});
  void run();
})();
