...
window.FenixCompletePicture=(()=>{
  ...
  function drawCustom(ctx,image,x,y,scale,guide=false){if(!image)return;const maxW=430*scale,maxH=560*scale,ratio=Math.min(maxW/image.naturalWidth,maxH/image.naturalHeight),w=image.naturalWidth*ratio,h=image.naturalHeight*ratio;ctx.save();ctx.filter=guide?"grayscale(1) contrast(.55)":"grayscale(1) contrast(1.12)";ctx.drawImage(image,x-w/2,y-h/2,w,h);ctx.restore()}
  ...
  function render(options={},seed="fenix-ctp",pageNo=0,{solution=false,scale=1,customImage=null}={}){
    ...
    const drawCurrent=(x,y,s,alpha=1)=>{ctx.save();ctx.globalAlpha=Math.max(0,Math.min(1,alpha));o.assetSource==="upload"?drawCustom(ctx,customImage,x,y,s,false):drawBuiltIn(ctx,o.asset,x,y,s);ctx.restore()};
    const drawGuide=(x,y,s,alpha)=>{ctx.save();ctx.globalAlpha=Math.max(.02,Math.min(.35,alpha));ctx.strokeStyle="#9aa3ab";ctx.lineWidth=Math.max(1,Math.min(3,o.lineWidth*.55));o.assetSource==="upload"?drawCustom(ctx,customImage,x,y,s,true):drawBuiltIn(ctx,o.asset,x,y,s);ctx.restore()};
    const guideAlpha=o.missingGuide==="none"?0:Math.max(.02,Math.min(.35,o.missingGuideOpacity*(o.missingGuide==="very-faint"?.65:1)));
    if(o.type==="shadow-trace"){const traceScale=o.scale*Math.max(.8,Math.min(1.9,o.shadowScale||1.4));drawGuide(cx,555,traceScale,solution?1:Math.max(.03,Math.min(.35,o.shadow)))}
