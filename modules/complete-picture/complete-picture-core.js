...
window.FenixCompletePicture=(()=>{
  ...
  function render(options={},seed="fenix-ctp",pageNo=0,{solution=false,scale=1,customImage=null}={}){
    ...
    if(o.type==="shadow-trace"){const traceScale=o.scale*Math.max(.8,Math.min(1.9,o.shadowScale||1.4));solution?drawCurrent(cx,555,traceScale,1):drawGuide(cx,555,traceScale,Math.max(.03,Math.min(.25,o.shadow)))}
