...
window.FenixCompletePicture=(()=>{
  ...
  function render(options={},seed="fenix-ctp",pageNo=0,{solution=false,scale=1,customImage=null}={}){
    ...
    if(o.type==="shadow-trace"){const traceScale=o.scale*Math.max(.8,Math.min(1.9,o.shadowScale||1.4));drawCurrent(cx,555,traceScale,solution?1:Math.max(.03,Math.min(.35,o.shadow)))}
