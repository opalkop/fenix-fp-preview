...
(()=>{
  function install(paneId,gridId,placeholder){
    ...
    if(!pane||!grid||pane.querySelector('.ctp-asset-search'))return false;
    const input=document.createElement('input');
    input.type='search';
    input.className='ctp-asset-search';
    ...
    const filter=()=>{
      ...
      grid.querySelectorAll('.asset-card').forEach(card=>{
        const text=(card.querySelector('strong')?.textContent||card.textContent||'').toLocaleLowerCase('pl');
