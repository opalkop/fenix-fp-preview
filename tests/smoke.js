...
(()=>{
 ...
 function run(){
   ...
   test(registry,"15 unikalnych modułów",()=>{const all=FenixModuleRegistry.all(),slugs=all.map(x=>x.slug);assert(all.length===15,`Jest ${all.length}`);assert(new Set(slugs).size===slugs.length,"Powtarzające się slug-i");return `${all.length} modułów`});
   test(registry,"7 standardowych rendererów",()=>{const standard=FenixModuleRegistry.standard();assert(standard.length===7,`Jest ${standard.length}`);standard.forEach(module=>assert(FenixStandardRenderers.modules.includes(module.slug),`Brak renderera ${module.slug}`));return "wszystkie podpięte"});
   ...
   test(typography,"Coloring Studio zapisuje referencję do assetu",()=>{const page=FenixPageSchema.normalize({module:"coloring-studio",title:"Color!",recipe:{module:"coloring-studio",settings:{titleSize:112,instructionSize:46,assetScale:82},content:{assetRef:"asset-ocean"}}});const options=FenixColoring.optionsFromPage(page);assert(options.assetRef==="asset-ocean","Zginęła referencja do assetu");assert(options.titleSize>=100&&options.instructionSize>=40,"Za mała typografia");return `${options.assetScale}%`});
