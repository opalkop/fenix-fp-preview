...
(()=>{
  ...
  const fileInput=$("#projectAssetFile"),grid=$("#projectAssetsGrid"),empty=$("#projectAssetsEmpty"),status=$("#projectAssetsStatus"),count=$("#projectAssetsCount"),search=$("#projectAssetSearch"),tagFilter=$("#projectAssetTagFilter"),statusFilter=$("#projectAssetStatusFilter"),sort=$("#projectAssetSort"),visibleCount=$("#projectAssetsVisibleCount");
  ...
  const roleLabel=(tag,active)=>`${active?"✓ ":""}${({gameplay:"Gameplay",content:"Content",deco:"Deco"}[tag]||tag)}`;
  ...
  function toggleTag(asset,tag){const tags=new Set(asset.tags||asset.meta?.tags||[]),wasActive=tags.has(tag);wasActive?tags.delete(tag):tags.add(tag);FenixCore.updateAsset(asset.id,{tags:[...tags]});setStatus(`${wasActive?"Usunięto":"Przypisano"} rolę ${roleLabel(tag,false)} dla „${asset.name}”.`,"ok")}
  ...
  function remove(asset){const usage=FenixCore.assetUsage(asset.id);if(usage.length){setStatus(`Nie usunięto „${asset.name}”. Asset jest używany przez ${usage.length} ${usage.length===1?"stronę":"strony"} projektu.`,"warning");return}
    ...
    const result=FenixCore.removeAsset(asset.id);if(result?.removed){setStatus("Asset został usunięty.","ok")}else setStatus("Nie udało się usunąć assetu.","error")
  }
  ...
  function tagButton(tag,tags,title){const active=tags.has(tag);return `<button type="button" data-tag="${tag}" class="${active?"active":""}" aria-pressed="${active}" title="${title}">${roleLabel(tag,active)}</button>`}
  function render(){const all=FenixCore.listAssets(),assets=filteredAssets();if(count)count.textContent=`${all.length} ${all.length===1?"asset":"assetów"}`;if(visibleCount)visibleCount.textContent=all.length===assets.length?"":`Widoczne: ${assets.length} z ${all.length}`;if(empty){empty.hidden=all.length>0;const noMatches=$("#projectAssetsNoMatches");if(noMatches)noMatches.hidden=all.length===0||assets.length>0}if(!grid)return;grid.innerHTML="";
    assets.forEach(asset=>{const card=document.createElement("article");card.className="project-asset-card";const tags=new Set(asset.tags||asset.meta?.tags||[]),validation=asset.validation||asset.meta?.validation||{status:"warning",messages:["Asset nie był jeszcze walidowany."]};const dims=asset.width&&asset.height?`${asset.width}×${asset.height}px`:"wymiary —",ratio=asset.aspectRatio?`1:${(1/Number(asset.aspectRatio)).toFixed(2)}`:"—",usage=FenixCore.assetUsage(asset.id);card.innerHTML=`<div class="project-asset-thumb"><img src="${asset.dataUrl}" alt="${escapeHtml(asset.name||"Asset")}"><span class="asset-source">${escapeHtml(sourceLabel(asset.source))}</span></div><div class="project-asset-info"><div class="project-asset-title"><strong title="${escapeHtml(asset.name||"Asset")}">${escapeHtml(asset.name||"Asset")}</strong><span class="asset-validation ${escapeHtml(validation.status||"warning")}">${statusLabel(validation.status)}</span></div><small>${escapeHtml(asset.filename||asset.mime||"asset")} · ${dims} · ${fmtBytes(asset.sizeBytes)}</small><small>Proporcje: ${escapeHtml(String(asset.aspectRatio||ratio))} · użycia na stronach: ${usage.length}</small><div class="asset-tags" aria-label="Role assetu">${tagButton("gameplay",tags,"Asset może brać udział w zadaniu")}${tagButton("content",tags,"Asset może być główną treścią strony")}${tagButton("deco",tags,"Asset może służyć jako dekoracja")}</div><p class="asset-message">${escapeHtml((validation.messages||[])[0]||"Gotowy do użycia w projekcie.")}</p><div class="asset-actions"><button data-action="rename">Zmień nazwę</button><button data-action="validate">Sprawdź B&W</button><button data-action="remove" class="danger" ${usage.length?"disabled title=\"Asset jest używany przez strony projektu\"":""}>Usuń</button></div></div>`;
      card.querySelectorAll("[data-tag]").forEach(button=>button.onclick=()=>toggleTag(asset,button.dataset.tag));card.querySelector('[data-action="rename"]').onclick=()=>rename(asset);card.querySelector('[data-action="validate"]').onclick=()=>revalidate(asset);card.querySelector('[data-action="remove"]').onclick=()=>remove(asset);grid.appendChild(card)});
  }
